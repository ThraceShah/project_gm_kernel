...
- 30 个 case-group，327 个成功 manifest；全部通过真实 Parasolid 创建、XT transmit/receive
  和组内 `--check`。
- required semantic cells：250；成功覆盖 226；精确拒绝 9；归一化 10。
- 当前 `missingLabels` 有 5 个：`geometry.blend.depth.2`、
  `geometry.blend.pair.sphere-spun`、`geometry.icurve.depth.2`、
  `topology.loop.hole`、`topology.loop.peripheral`。其中后三个有固定 seed 的
  `needs-recipe` 记录，前两个 depth-2 单元等待稳定的嵌套构造配方。
- `deferredLabels` 仍有 17 个，表示计划层面的组合矩阵或 callback/运行时能力尚未完全
  关闭；typed strict gap 为 5，不能宣称计划全部完成。
...
- ICurve：12 个成功 fixture，覆盖 BSurf 与 Cylinder/Cone/Sphere/Torus/Swept/Spun、
  Cone/Swept、Sphere/Swept，以及反向 face 顺序、seed vector、finite box、
  periodic support、face sense。其余解析交线若被内核表示为 Line/Circle/Ellipse，不冒充
  `PK_CLASS_icurve`，记录在 `coverage/unreachable/icurve-surface-matrix.json`。
- Face-face blend：19 个成功 matrix case，覆盖多个 supporting-surface 配对代表、
  rolling-ball、reversed sense、conic、multiple、propagate、relative rho、help point，
  并检查 `BLENDED_EDGE`/`BLEND_BOUND`/`INTERSECTION`。sphere/spun 与 blend-on-blend
  的固定 seed 分别记录为 `needs-recipe`；notch 固定 seed 无返回 blend body，记录为精确
  拒绝；width/ratio 的固定 seed 精确返回 `PK_fxf_fault_inconsistent (17455)`。
...
1. `PK_LOOP_type_hole_c (5401)` 和 `PK_LOOP_type_peripheral_c (5402)` 的可 transmit/receive
   公共 API 构造配方；当前 v38 runtime 分别返回 `inner`/`outer`。
2. `sphere-spun` face-face blend 和 blend-on-blend 深度 2 的稳定参数 spine；当前固定
   seed 分别返回 fxf fault 17462/17461。
3. 若必须关闭全部 ICurve/Blend 组合，请提供能让解析面交线或 derived-surface payload
   保持 `PK_CLASS_icurve`/可解码 XT 的固定 seed；否则这些合法数据仍应保留 `needs-recipe`。
4. CPCurve、Frame、Attribute/User-field callback 若有许可内 producer 或 callback ABI，
