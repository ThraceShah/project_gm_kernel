...
var header = GenerateHeader(versions);
...
WriteOrCheck(Path.Combine(repositoryRoot, "src", "ProjectGmKernel.Xt.Native", "include", "ProjectGmKernel.Xt.Schema.generated.h"), header);
...
string GenerateHeader(List<VersionSchema> schemas)
{
    var output = new StringBuilder().AppendLine("/* <auto-generated/> schema-derived types; no schema source text is embedded. */").AppendLine("#ifndef PROJECT_GM_KERNEL_XT_SCHEMA_GENERATED_H").AppendLine("#define PROJECT_GM_KERNEL_XT_SCHEMA_GENERATED_H").AppendLine("#include \"ProjectGmKernel.Xt.h\"");
    foreach (var version in schemas)
    {
        ...
        {
            var fields = version.Definition.GetFields(node).ToArray();
            output.Append("typedef struct ").Append(prefix).Append('_').Append(node.Name).AppendLine("_s {");
            ...
            foreach (var field in fields)
            {
                ...
                output.Append("    ").Append(type).Append(' ').Append(field.Name);
                if (field.ElementCount > 1) output.Append('[').Append(field.ElementCount).Append(']');
                output.AppendLine(";");
            }
            output.Append("} ").Append(prefix).Append('_').Append(node.Name).AppendLine("_t;");
        }
        output.Append("typedef struct ").Append(prefix).AppendLine("_COUNTS_s {");
        ...
        output.Append("} ").Append(prefix).AppendLine("_COUNTS_t;");
        output.Append("PGM_XT_API PGM_XT_status_t ").Append(prefix).Append("_MODEL_create(const ").Append(prefix).AppendLine("_COUNTS_t *, PGM_XT_model_t *);");
        output.Append("PGM_XT_API PGM_XT_status_t ").Append(prefix).AppendLine("_MODEL_finalize(PGM_XT_model_t);");
        output.Append("PGM_XT_API PGM_XT_status_t ").Append(prefix).AppendLine("_MODEL_delete(PGM_XT_model_t);");
        output.Append("PGM_XT_API PGM_XT_status_t PGM_XT_DOCUMENT_to_").Append(version.Identity).AppendLine("_MODEL(PGM_XT_document_t, PGM_XT_model_t *);");
        output.Append("PGM_XT_API PGM_XT_status_t PGM_XT_").Append(version.Identity).AppendLine("_MODEL_to_DOCUMENT(PGM_XT_model_t, PGM_XT_document_t *);");
        foreach (var node in version.Definition.Nodes.ToArray().Where(static node => node.Transmit))
        {
            output.Append("PGM_XT_API PGM_XT_status_t ").Append(prefix).Append('_').Append(node.Name).Append("_get_read_view(PGM_XT_model_t, const ").Append(prefix).Append('_').Append(node.Name).AppendLine("_t **, int32_t *);");
            output.Append("PGM_XT_API PGM_XT_status_t ").Append(prefix).Append('_').Append(node.Name).Append("_get_write_view(PGM_XT_model_t, ").Append(prefix).Append('_').Append(node.Name).AppendLine("_t **, int32_t *);");
            foreach (var field in version.Definition.GetFields(node).ToArray().Where(static field => field.Transmit && field.ElementCount == 1))
            {
                output.Append("PGM_XT_API PGM_XT_status_t ").Append(prefix).Append('_').Append(node.Name).Append('_').Append(field.Name).Append("_get_read_view(PGM_XT_model_t, const PGM_XT_field_").Append(field.Type).AppendLine("_t **, int32_t *);");
                output.Append("PGM_XT_API PGM_XT_status_t ").Append(prefix).Append('_').Append(node.Name).Append('_').Append(field.Name).Append("_get_write_view(PGM_XT_model_t, PGM_XT_field_").Append(field.Type).AppendLine("_t **, int32_t *);");
            }
        }
    }
...
string GenerateMapping(List<VersionSchema> schemas)
{
    ...
    foreach (var version in schemas)
    {
        output.Append("## ").Append(version.Identity).AppendLine().AppendLine().AppendLine("| Node | Type | Transmit | Fields | Managed namespace | C prefix |").AppendLine("|---|---:|---:|---:|---|---|");
        foreach (var node in version.Definition.Nodes)
            output.Append('|').Append(node.Name).Append('|').Append(node.Type).Append('|').Append(node.Transmit ? 1 : 0).Append('|').Append(node.ParsedFieldCount).Append("|`ProjectGmKernel.Xt.Schema.").Append(Namespace(version.Identity)).Append('.').Append(node.Name).Append("`|`PGM_XT_").Append(version.Identity).Append('_').Append(node.Name).AppendLine("_t`|");
